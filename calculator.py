...
# y:        　　   组件右上角的y坐标；
# 放置清除按钮和等号按钮
btnClear = tkinter.Button(root, text='C', bg='red', command=lambda: button(
...
btnCompute = tkinter.Button(root, text='=', bg='yellow', command=lambda: button('='))
...
for row in range(4):
    for col in range(3):
        ...
        index += 1  # 索引号递增
        btnDigit = tkinter.Button(root, text=d, command=lambda x=d: button(x))  # 和上面的是类似的
